...
from framework import MultiResolutionSTFTLoss, SPKDLoss, build_review_kd, hcl
...
class KnowledgeDistillation(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        feature_maps_loss['encoder'] = hcl(student_features_encoder,teacher_encoder)
        ...
        feature_maps_loss['decoder'] = hcl(student_features_decoder,teacher_decoder)


        ############## C-LSTM REAL loss ######################
        # calculating review kd loss
        print(student_clstm_real.shape)
        kd_loss = hcl(student_clstm_real, teacher_clstm_real)
        feature_maps_loss['clstm_real'] = kd_loss
        

        ############## C-LSTM IMAGE loss ######################
        # calculating review kd loss
        kd_loss = hcl(student_clstm_img, teacher_clstm_img)
...
checkpoint_callback = ModelCheckpoint(
                    dirpath='/root/NTH_student/Speech_Enhancement_new/knowledge_distillation_CLSKD/checkpoint_ReviewKD',
...
torch.save(to_save,os.path.join('/root/NTH_student/Speech_Enhancement_new/knowledge_distillation_CLSKD/checkpoint_ReviewKD', "best_model_ReviewKD.pth"))
